...
func getComplexSliceParam(k string, v []string, curParams *genlib.ComplexSliceParams) error {
	switch k {
	case "dimensions":
...
func getComplexParam(k string, v []string, curParams *genlib.ComplexParams) error {
	switch k {
	case "real_values":
		strList := strings.Split(v[0], ",")
		for _, v := range strList {
